...
int APIENTRY WinMain(HINSTANCE hI,HINSTANCE hP,LPSTR lp,int nC)
{
   ...
   wc.lpszClassName = "W1";
   RegisterClass(&wc);
   hWnd=CreateWindowEx(WS_EX_PALETTEWINDOW,"W1","UDP 程序1 [端口:8000]",
                       WS_OVERLAPPEDWINDOW,100,100,200,100,NULL,NULL,hI,NULL);
    if(!hWnd)
        return FALSE;
    ShowWindow(hWnd,nC);
...
LRESULT CALLBACK WndProc(HWND hW,UINT msg,WPARAM wP,LPARAM lP)
{
    ...
    case WM_USER+1:
        ...
        case FD_READ:
            ...
            recvBuf[len] = 0;
            if(len!=-1)
                MessageBox(0,recvBuf,"UDP 程序1从UDP 程序2收到的数据",MB_OK);

            break;
        }
        break;
    case WM_LBUTTONDOWN:
        wsprintf(sendBuf,"Hello,UDP 程序2!");
        len = sizeof(Cs2A);
